...
export class CompanyRepositoryImp implements CompanyRepository {
    ...
    async create(company: DomainCompany): Promise<DomainCompany> {
        const response = await this.http.POST<DomainCompany, DomainCompany>("companies", company)
        return response.data
    }

    async getOne(uuid: string): Promise<DomainCompany> {
        const response = await this.http.GET<DomainCompany>(`companies/${uuid}`)
        return response.data
    }

    async getAll(query?: IQueryMetadata): Promise<DomainCompany[]> {
        const response = await this.http.GET<DomainCompany[]>("companies", { ...query })
        return response.data
    }

    async update(company: DomainCompany): Promise<DomainCompany> {
        const response = await this.http.PUT<DomainCompany, DomainCompany>(`companies/${company.uuid}`, company)
        return response.data
    }
