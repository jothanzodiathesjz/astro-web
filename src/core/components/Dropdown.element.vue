<template>
  <div
    ...
    :class="refined_options.length > 0 ? 'border-blue-300' : 'border-red-300'"
    ref="root"
  >
    ...
    >
      <button
        ...
        :class="selected?.id === option.id ? 'font-bold selected-dropdown' : ''"
        @click.prevent="$emit('select', option)"
      >
        <div class="flex flex-col">
          <span>{{ option.label }}</span>
          <span class="text-xs text-gray-400" v-if="option.subtitle">{{
            option.subtitle
          }}</span>
        </div>
      </button>
