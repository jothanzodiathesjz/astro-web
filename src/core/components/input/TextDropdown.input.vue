<template>
    <div class="w-full flex flex-col gap-1">
        <span v-if="label" class="dark:text-gray-200 text-sm">{{ label }} <span v-if="!isOptional" class="text-xs text-red-600 dark:text-red-400">*</span></span>
        <div
            ref="dropdownContainer"
            class="relative flex flex-row gap-4 border bg-white dark:bg-gray-800 items-center rounded-lg"
            :class="
                showDropdown ? ' border-blue-200 dark:border-blue-500' : 'border-gray-300 dark:border-gray-700'
            "
        >
            <div class="px-3 flex items-center gap-3 justify-between w-full">
                ...
                />
                <!-- <span
                    class="material-icons text-neutral-60 transition-all"
                    :class="showDropdown ? 'rotate-180' : ''"
             ...
             <Transition name="slide">
                 ...
                 >
                     ...
                     >
                         <div
                             class="hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-800 text-sm dark:text-gray-200 rounded px-3 py-3 w-full"
                             :class="
                                 selected?.id === option.id ? 'font-semibold bg-blue-100' : ''
                             "
                     ...
                     <li v-else>
                         <div class="w-full flex flex-col gap-1 items-center py-3">
                             <span class="text-sm dark:text-gray-200">No Result Found</span>
                             <ButtonComponent
                                 v-if="adding_button"
                                 :variant="'outline'"
                                 :icon="{
