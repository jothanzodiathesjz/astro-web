        ...
        >
            <div class="w-full flex flex-row">
                ...
                <div class="w-full flex flex-row gap-4 justify-end">
                    <ButtonComponent
                        @click="$router.push({ name: 'TimeOffPolicyForm' })"
                        variant="primary"
                        >Create Policy</ButtonComponent
                    >
                </div>
            </div>
...
import type { DomainTimeOffPolicy } from "@/domain/models/TimeOff";
...
const list = ref<DomainTimeOffPolicy[]>([]);
const search = ref<string>("");
const alerts = ref<ToastUI | null>(null);

async function getList() {
    try {
        list.value = await repository.getListPolicy({
            search: search.value,
        });
    } catch (error) {
        console.log(error);
