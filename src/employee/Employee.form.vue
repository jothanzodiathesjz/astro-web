<template>
  <div class="w-full h-full flex flex-col items-center px-14 dark:bg-gray-900 pb-6 text-gray-800">
    <Toast 
    ...
    <div class="w-[90%] flex flex-row items-center py-3 mt-3 gap-4">
      ...
      <ButtonComponent @click="$router.back()" :variant="'outline'" :icon-name="'fa-angle-left'">Back</ButtonComponent>
    ...
    >
      ...
      <div class="w-full p-5 flex flex-col gap-3">
        <div v-if="vm.steps === 1" class="w-full flex flex-col gap-4">
          <span class="font-semibold dark:text-gray-200">Personal Detail</span>
          <div class="flex flex-row gap-5 items-end">
            <TextInput
              ...
              @input="(v) => (vm.employee.personalDetail.firstName = v)"
            />
            <TextInput
              label="Last Name"
              ...
              @input="(v) => (vm.employee.personalDetail.lastName = v)"
            />
          </div>
          <div class="flex flex-row gap-5 items-start">
             <TextInput
              ...
              @input="(v) => (vm.employee.personalDetail.nickName = v)"
            />
            <div class="flex flex-col gap-1 w-full">
              <span class="text-sm">Birth Date <span class="text-red-500">*</span></span>
              <div class="flex flex-col gap-2">
                <DateInput
                  :model-value="vm.employee.personalDetail.birthDate"
                  :with-range="false"
                  :placeholder="'Pilih Tanggal Lahir'"
                  :readonly="false"
                  :with-time="false"
                  @update:model-value="(value) => (vm.employee.personalDetail.birthDate = value)"
                />
                <span  class="text-xs text-red-500" v-if="vm.erorrs?.data?.birthDate">{{ vm.erorrs?.data?.birthDate }}</span>
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-5 items-start">
            <TextInput
              ...
              @input="(v) => (vm.employee.personalDetail.birthPlace = v)"
            />
            <div class="flex flex-col gap-2">
              <div class="flex flex-col gap-2">
                <TextDropdownInput
                  ...
                  :dontfilter="true"
                  @update="(value) => (vm.employee.personalDetail.bloodType = value.value)"
                  @clear="() => (vm.employee.personalDetail.bloodType = null)"
                />
                <span class="text-xs text-red-500 te" v-if="vm.erorrs?.data?.bloodType">{{ vm.erorrs?.data?.bloodType }}</span>
              </div>
            </div>
          ...
          <div class="grid grid-cols-2 gap-5 items-start">
            <div class="flex flex-col gap-2">
              <TextDropdownInput
                ...
                :dontfilter="true"
                @update="(value) => (vm.employee.personalDetail.religion = value.value)"
                @clear="() => (vm.employee.personalDetail.religion = null)"
              />
              <span  class="text-xs text-red-500" v-if="vm.erorrs?.data?.religion">{{ vm.erorrs?.data?.religion }}</span>
            </div>
            <div class="flex flex-col gap-2">
              <TextDropdownInput
                label="Status"
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.personalDetail.maritalStatus = value.value)"
                @clear="() => (vm.employee.personalDetail.maritalStatus = null)"
              />
              <span  class="text-xs text-red-500" v-if="vm.erorrs?.data?.maritalStatus">{{ vm.erorrs?.data?.maritalStatus }}</span>
            </div>
            <div class="flex flex-col gap-2 items-start">
              <div class="flex flex-col gap-2">
                <span class="text-sm dark:text-gray-400">Gender <span class="text-red-500">*</span></span>
                <div class="flex flex-row gap-16 shrink-0 py-2">
                  ...
                  >
                    <button
                      ...
                      @click="
                        vm.employee.personalDetail.gender = gender === 'MALE' ? 'M' : 'F'
                      "
                    >
                      <span
                        :class="
                    ...
                    <span class="dark:text-gray-400 text-sm">{{ gender }}</span>
                  </div>
                </div>
              </div>
              <span class="text-xs text-red-500" v-if="vm.erorrs?.data?.gender">{{ vm.erorrs?.data?.gender }}</span>
            </div>
          </div>
        </div>
        <div v-if="vm.steps === 2" class="w-full flex flex-col gap-4">
          <span class="font-semibold dark:text-gray-200">Address</span>
          <div class="flex flex-row gap-5 items-end">
            <TextInput
              optional="true"
              label="Country"
        ...
        <div v-if="vm.steps === 3" class="w-full flex flex-col gap-4">
          <span class="font-semibold dark:text-gray-200">Contact Detail</span>
          <div class="flex flex-row gap-5 items-start">
            <TextInput
              label="Email"
              ...
              @input="(v) => (vm.employee.contactDetail.email = v)"
            />
            <div class="w-full flex flex-col gap-2 items-start">
              <PhoneInput 
              :label="'Phone Number (Whatsapp)'" 
              :phone="vm.employee.contactDetail.phoneNumber" 
              @input="(v) => (vm.employee.contactDetail.phoneNumber = new PhoneUI(v))"
              />
              <span v-if="vm.erorrs?.data?.phoneNumber"  class="text-xs text-red-500">{{ vm.erorrs?.data?.phoneNumber }}</span>
            </div>
          ...
          <div class="grid grid-cols-2 gap-5 items-end">
            <TextInput label="Name" :value="vm.employee.contactDetail.emergencyName" :placeholder="'Name'" :error="vm.erorrs?.data?.emergencyName" @input="(v) => (vm.employee.contactDetail.emergencyName = v)" />
            <div class="flex flex-col gap-2">
              <TextDropdownInput
                label="Relasi"
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.contactDetail.emergencyRelation = value.value)"
                @clear="() => (vm.employee.contactDetail.emergencyRelation  = null)"
              />
              <span v-if="vm.erorrs?.data?.emergencyRelation"  class="text-xs text-red-500" >{{ vm.erorrs?.data?.emergencyRelation }}</span>
            </div>
          </div>
          <div class="flex flex-col gap-2 items-start">
            ...
            <PhoneInput 
            :label="'Phone Number (Whatsapp)'" 
            :phone="vm.employee.contactDetail.emergencyPhoneNumber"  
            @input="(v) => (vm.employee.contactDetail.emergencyPhoneNumber = new PhoneUI(v))"
            />
            <span v-if="vm.erorrs?.data?.emergencyPhoneNumber"  class="text-xs text-red-500" >{{ vm.erorrs?.data?.emergencyPhoneNumber }}</span>
          </div>
        </div>
        <div v-if="vm.steps === 4" class="w-full flex flex-col gap-4">
          ...
          <div class="flex flex-row gap-5 items-start">
            ...
            @input="(v) => (vm.employee.employmentDetail.employeeId = v)"
            ...
            @input="(v) => (vm.employee.employmentDetail.fingerCode = v)"
            />
          </div>
          <div class="flex flex-row gap-5 items-start">
            <div class="w-full flex flex-col gap-2">
              <TextDropdownInput
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.employmentDetail.department = value.value)"
                @clear="() => (vm.employee.employmentDetail.department = null)"
              />
              <span v-if="vm.erorrs?.data?.department"  class="text-xs text-red-500">{{ vm.erorrs?.data?.department }}</span>
            </div>
            ...
            <div class="w-full flex flex-col gap-2">
              <TextDropdownInput
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.employmentDetail.jobLevel = value.value)"
                @clear="() => (vm.employee.employmentDetail.jobLevel = null)"
              />
              <span v-if="vm.erorrs?.data?.jobLevel"  class="text-xs text-red-500">{{ vm.erorrs?.data?.jobLevel }}</span>
            </div>
          ...
          <div class="flex flex-row gap-5 items-start">
            <div class="w-full flex flex-col gap-2">
               ...
               @input="(v) => (vm.employee.employmentDetail.jobTitle = v)"
               />
            </div>
            <div class="w-full flex flex-col gap-2">
              <TextDropdownInput
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.employmentDetail.employmentType = value.value)"
                @clear="() => (vm.employee.employmentDetail.employmentType = null)"
              />
              <span v-if="vm.erorrs?.data?.employmentType"  class="text-xs text-red-500">{{ vm.erorrs?.data?.employmentType }}</span>
            </div>
            
          </div>
          <div class="flex flex-row gap-5 items-start">
            <div class="w-full flex flex-col gap-2">
              <span class="text-sm dark:text-gray-400">Start Date <span class="text-red-500 text-xs">*</span></span>
              <DateInput
                :model-value="vm.employee.employmentDetail.startDate"
                :with-range="false"
                :placeholder="'Start date'"
                :readonly="false"
                :with-time="false"
                @update:model-value="(value) => (vm.employee.employmentDetail.startDate = value instanceof Date ? value : undefined)"
              />
              <span v-if="vm.erorrs?.data?.startDate"  class="text-xs text-red-500">{{ vm.erorrs?.data?.startDate }}</span>
            </div>
            <div class="w-full flex flex-col gap-2">
              <span class="text-sm dark:text-gray-400">End Date</span>
              <DateInput
                :model-value="vm.employee.employmentDetail.endDate"
                :with-range="false"
                :placeholder="'End date'"
                :readonly="false"
                :with-time="false"
                @update:model-value="(value) => (vm.employee.employmentDetail.endDate = value instanceof Date ? value : undefined)"
              />
              <span v-if="vm.erorrs?.data?.endDate"  class="text-xs text-red-500">{{ vm.erorrs?.data?.endDate }}</span>
            </div>
            
          </div>
          <div class="flex flex-row gap-5 items-start">
            <div class="w-full flex flex-col gap-2">
              <span class="text-sm dark:text-gray-400">Resign Date</span>
              <DateInput
                :model-value="vm.employee.employmentDetail.resignDate"
                :with-range="false"
                :placeholder="'Resign date'"
                :readonly="false"
                :with-time="false"
                @update:model-value="(value) => (vm.employee.employmentDetail.resignDate = value instanceof Date ? value : undefined)"
              />
              <span v-if="vm.erorrs?.data?.resignDate"  class="text-xs text-red-500">{{ vm.erorrs?.data?.resignDate }}</span>
            </div>
            <div class="w-full flex flex-col gap-2">
              <span class="text-sm dark:text-gray-400">Status <span class="text-red-500 text-xs">*</span></span>
              <TextDropdownInput
                ...
                :dontfilter="true"
                :with-clear="true"
                @update="(value) => (vm.employee.employmentDetail.status = value.value)"
                @clear="() => (vm.employee.employmentDetail.status = null)"
              />
              <span v-if="vm.erorrs?.data?.status"  class="text-xs text-red-500">{{ vm.erorrs?.data?.status }}</span>
            </div>
          </div>
        </div>
        <div v-if="vm.steps === 5" class="w-full flex flex-col gap-4">
          <span class="font-semibold dark:text-gray-200">Bank Detail</span>
          <div class="flex flex-row gap-5 items-start">
            <TextInput 
            label="Account Name" 
            ...
            @input="(v) => (vm.employee.bankDetail.accountName = v)"
            />
            <TextInput 
            ...
            @input="(v) => (vm.employee.bankDetail.accountNumber = v)"
            />
          </div>
          <div class="flex flex-row gap-5 items-start">
            <TextInput 
            ...
            @input="(v) => (vm.employee.bankDetail.bankName = v)"/>
            <TextInput 
            label="IFSC Code" 
            :value="vm.employee.bankDetail.ifscCode" 
            :placeholder="'IFSC Code'" 
            :error="vm.erorrs?.data?.ifscCode"
            @input="(v) => (vm.employee.bankDetail.ifscCode = v)"
            />
          </div>
        </div>
        <div v-if="vm.steps === 6" class="w-full flex flex-col gap-4">
          <span class="font-semibold dark:text-gray-200">Insurance</span>
          <div class="flex flex-row gap-5 items-start">
            <TextInput 
            label="No Bpjs Ketenagakerjaan" 
            :value="vm.employee.insuranceDetail.bpjsKetenagakerjaan" 
            :placeholder="'Bpjs Ketenagakerjaan'" 
            :error="vm.erorrs?.data?.bpjsKetenagakerjaan"
            @input="(v) => (vm.employee.insuranceDetail.bpjsKetenagakerjaan = v)"
            />
            <TextInput 
            ...
            @input="(v) => (vm.employee.insuranceDetail.bpjsKesehatan = v)"
            />
          </div>
        </div>
        <div class="w-full flex flex-row justify-end mt-4 gap-4">
          ...
          >
          <ButtonComponent v-if="vm.steps === 6" :variant="'primary'"
...
const selectedDepartment = computed(() => {
  return vm.value.employee.employmentDetail.department
    ? new DropdownLabel(vm.value.employee.employmentDetail.department, "code", "name")
    : null;
})
